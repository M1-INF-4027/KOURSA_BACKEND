...
from .serializers import ValidationTokenSerializer
...
class FicheSuiviViewSet(viewsets.ModelViewSet):
    ...
    def valider(self, request, pk=None):
        ...
        if fiche.statut != StatutFiche.SOUMISE:
            return Response({"detail": "Cette fiche ne peut plus être validée."}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def refuser(self, request, pk=None):
        ...
        if fiche.statut != StatutFiche.SOUMISE:
            return Response({"detail": "Cette fiche ne peut plus être refusée."}, status=status.HTTP_400_BAD_REQUEST)

        motif = request.data.get('motif')
        if not motif:
            return Response({"motif": ["Ce champ est obligatoire."]}, status=status.HTTP_400_BAD_REQUEST)
